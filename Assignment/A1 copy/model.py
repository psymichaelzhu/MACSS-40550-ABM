...
# [modification] @ line 10~20: helpers used in the datacollector to collect the boundary mixing and segregation
#<<<<<<<<<<<<<<<<<<<<<<<
...
class SchellingModel(Model):
    ...
    def __init__(self, growth_rate = 0.1, time_horizon = 3, width = 30, height = 30, density = 0.7, satisfaction_threshold = 0.5, group_one_share = 0.7, radius = 1, seed = None):
        ...
        # [modification] introduce agent id, used to identify the agent in the contact matrix.
        # I also noticed that there is a build-in parameter "unique_id" in the Agent class
        #<<<<<<<<<<<<<<<<<<<<<<<
        agent_id = 0 
        ...
        self.datacollector = DataCollector(
            model_reporters = {
                ...
                # [modification] @ line 35~38: more metrics to collect
                #<<<<<<<<<<<<<<<<<<<<<<<
