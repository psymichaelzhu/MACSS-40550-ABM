...
to some extent, 传统schelling model仅考虑当下，contact model考虑了memory of history， growth mindset model考虑了furture planning

[TODO] 润色implementation note
...
[TODO] 完善results 润色语言 通顺即可 不要花哨 不要太多
When we consider a high satisfaction threshold (0.9),
1. In the *classic Schelling model* (growth rate = 0, time horizon = 0), the agents keep moving around the grid and never settle down，share happy围绕在10%左右.
2. In the *contact model* (growth rate = 0.4, time horizon = 10), share happy逐步上升，在较长的时间后 settle down, end up in 明显的segregation.
